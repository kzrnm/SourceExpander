...
public class EmbedderGenerator : IIncrementalGenerator
{
    ...
    internal void Execute(IContextWrappter ctx, CSharpCompilation compilation, CSharpParseOptions parseOptions, AnalyzerConfigOptions analyzerConfigOptions, EmbedderConfig.Builder configBuilder)
    {
        try
        {
            ...
            if (!config.Enabled)
                return;
            if (analyzerConfigOptions.TryGetValue("build_property.SourceExpander_Embedder_VersionOnly", out var versionOnly)
                && StringComparer.OrdinalIgnoreCase.Equals(versionOnly, "true"))
            {
                ctx.AddSource(
                    "EmbeddedSourceCode.Metadata.cs", SourceText.From(
                        // lang=C#
                        """
                        // <auto-generated/>
                        #pragma warning disable
                        [assembly: global::System.Reflection.AssemblyMetadataAttribute("SourceExpander.EmbedderVersion",
                        """ + AssemblyUtil.AssemblyVersion.ToString().ToLiteral() + ")]", Encoding.UTF8));
                return;
            }
