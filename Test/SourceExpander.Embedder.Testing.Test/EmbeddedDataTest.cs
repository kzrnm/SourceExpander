...
public class EmbeddedDataTest
{
    ...
    public async Task GZipBase32768()
    {
        ...
        await embedded.AssemblyMetadatas.Keys.Should().BeEquivalentTo([
        ...
        await embedded.EmbeddedLanguageVersion.Should().BeEqualTo("6");
        await embedded.AssemblyMetadatas["SourceExpander.EmbeddedLanguageVersion"].Should().BeEqualTo("6");
        await embedded.EmbedderVersion.Should().BeEqualTo("4.0.2.100");
        await embedded.AssemblyMetadatas["SourceExpander.EmbedderVersion"].Should().BeEqualTo("4.0.2.100");
        await embedded.EmbeddedNamespaces.Should().BeEquivalentTo(["MathLibrary.Double", "SampleLibrary"]);
        await embedded.AssemblyMetadatas["SourceExpander.EmbeddedNamespaces"].Should().BeEqualTo("MathLibrary.Double,SampleLibrary");

        await embedded.SourceFiles.Should().BeEquivalentTo(ImmutableArray.Create<SourceFileInfo>([
            new(
    ...
    public async Task Raw()
    {
        ...
        var embedded = await GetEmbeddedDataAsync(dllPath);
        await embedded.AssemblyMetadatas.Keys.Should().BeEquivalentTo(["SourceExpander.EmbeddedSourceCode"]);
        await embedded.SourceFiles.Should().BeEquivalentTo(ImmutableArray.Create<SourceFileInfo>([
            new(
