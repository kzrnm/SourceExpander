...
{
    [Test]
    public async Task Allow(CancellationToken cancellationToken) => await Impl(true, [], cancellationToken);
    [Test]
    public async Task NotAllow(CancellationToken cancellationToken) => await Impl(false, [DiagnosticResult.CompilerWarning("EXPAND0010").WithArguments("/home/mine/Program.cs")], cancellationToken);

    static async Task Impl(bool allowUnsafe, DiagnosticResult[] expectedDiagnostics, CancellationToken cancellationToken)
    {
        var test = new Test
        {
